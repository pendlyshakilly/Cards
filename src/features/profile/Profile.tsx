...
const Profile = () => {
    ...
    const [name, setName] = useState('Hleb Sayanov')
		...
		return (
            ...
            <div className={s.Icon}><CameraAltIcon/></div>
							</div>
							{editMode ?
								<FormControl sx={{ m: 1, width: "347px", position: "relative" }} variant="standard">
										<InputLabel htmlFor="component-helper">Nickname</InputLabel>
										<Input
											style={{ width: "74%" }}
											id="component-helper"
											defaultValue={name}
											...
											autoFocus={true}
										/>
										<Button style={{ position: "absolute", top: "12px", right: "12px", fontWeight: '400'}}
														variant={"contained"} onClick={onClickHandler}>Save</Button>
								</FormControl>
								: <h3><span style={{fontWeight: '500', marginRight: '10px', fontSize: '20px'}}>{name}</span><BorderColorIcon onClick={onClickHandler}/></h3>
							}
							<p style={{color: 'gray'}}>j&johnson@gmail.com</p>
							<Button variant={'contained'} sx={{backgroundColor: '#FCFCFC', color: 'black', borderRadius: "40px"}}><LogoutIcon sx={{marginRight: '5px'}}/>Log out</Button>
					</Paper>
			</div>
		);
};
